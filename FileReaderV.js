...
FileReaderV.prototype.getFile = function(file, func, cors)
{
  ...
  {
    fetch((cors ? ("https://corsproxy.io/?url=" + encodeURIComponent(file)) : file), {cache: "no-cache"}).then(async function(r)
    {
...
{
  if( !this.temp ) { this.dataV.length = 0; this.dataV.offset = this.virtual; } else { this.tempD = []; this.tempD.length = 0; this.tempD.offset = this.virtual; }
